...
async function getProfile(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Errore nel recupero del profilo utente", error });
  }
}
...
async function getPublicProfile(req, res) {
  try {
    const user = await User.findById(req.params.id).select(
      "name surname username role status",
    );
    if (!user || user.status === "deleted")
      return res.status(404).json({ message: "Utente non trovato" });
...
async function updateProfile(req, res) {
  try {
    ...
    const { name, surname, username, email, currentPassword, newPassword } =
      req.body;
    ...
    if (newPassword) {
      ...
      if (!currentPassword)
        return res.status(400).json({
          message: "Inserire la password corrente per cambiare la password",
        });
      ...
      if (!isMatch)
        return res
          .status(400)
          .json({ message: "Password corrente non valida" });
    ...
    res
      .status(500)
      .json({ message: "Errore nell'aggiornamento del profilo utente", error });
  }
}
...
async function deleteProfile(req, res) {
  try {
    ...
    res.status(500).json({
      message: "Errore nella cancellazione del profilo utente",
      error,
    });
  }
}
