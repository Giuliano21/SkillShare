...
async function resolveTutorStudentPair(userAId, userBId) {
  ...
  const tutorProfile = await Tutor.findOne({ userId: tutorUserId }).select(
    "_id userId",
  );
...
async function createOrGetConversationByUsers(requesterId, peerUserId) {
  ...
  const canStartChat = await hasAcceptedBookingBetweenUsers(
    requesterId,
    peerUserId,
  );
  if (!canStartChat) {
  ...
  try {
    ...
    conversation = await Conversation.create({
      participants: [
        ...
        new mongoose.Types.ObjectId(peer),
      ],
      participantsKey,
    });
    return conversation;
...
async function ensureConversationMembership(conversationId, userId) {
  ...
  const isMember = conversation.participants.some(
    (participantId) => normalizeId(participantId) === userIdString,
  );
