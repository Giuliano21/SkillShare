  ...
  return (
    <div className="content-page narrow-page">
      ...
      <div className="my-review-list">
        {reviews.length ? (
          reviews.map((review) => (
            <article className="panel my-review" key={review._id}>
              <div className="section-heading">
                <div>
                  <p className="eyebrow">Tutor</p>
                  <h2>
                    {review.tutorId?.userId?.name}{" "}
                    {review.tutorId?.userId?.surname}
                  </h2>
                </div>
              ...
              <p>{review.comment}</p>
              <small>{review.bookingId?.subject || "Lezione"}</small>
              <div className="row-actions">
                <button
                  className="text-link"
                  onClick={() => {
          ...
          <p className="empty-state">Non hai ancora scritto recensioni.</p>
        )}
      </div>
    </div>
  );
