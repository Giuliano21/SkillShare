...
async function updateTutorRatingStats(tutorId) {
  ...
  const averageRating =
    reviewsCount > 0 ? (totalRating / reviewsCount).toFixed(1) : 0; // Se non ci sono recensioni, la media è 0
...
async function getReviews(req, res) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Recensioni recuperate con successo", reviews });
  } catch (err) {
    res.status(500).json({
  ...
  try {
    ...
    if (!validBooking) {
      return res.status(403).json({
        error:
          "Puoi recensire solo tutor con cui hai completato una prenotazione",
      });
    }
    ...
    return res
      .status(201)
      .json({ message: "Recensione creata con successo", review: newReview });
  } catch (err) {
    return res.status(500).json({
...
async function updateReview(req, res) {
  try {
    ...
    if (review.userId.toString() !== req.user.userId.toString()) {
      return res.status(403).json({
        message: "Non sei autorizzato ad aggiornare questa recensione",
      });
    }
    ...
    return res
      .status(200)
      .json({ message: "Recensione aggiornata con successo", review });
  } catch (err) {
    return res.status(500).json({
...
async function deleteReview(req, res) {
  try {
    ...
    if (review.userId.toString() !== req.user.userId.toString()) {
      return res.status(403).json({
        message: "Non sei autorizzato a cancellare questa recensione",
      });
    }
    ...
    return res
      .status(200)
      .json({ message: "Recensione cancellata con successo" });
  } catch (err) {
    return res.status(500).json({
