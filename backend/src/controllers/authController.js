...
async function register(req, res) {
  try {
    ...
    if (
      ...
      (!Array.isArray(req.body.lessonMode) ||
        req.body.lessonMode.length === 0 ||
        req.body.lessonMode.some(
          (mode) => !["remote", "presence"].includes(mode),
        ))
    ) {
      return res
        .status(400)
        .json({ message: "Selezionare almeno una modalità di lezione valida" });
    }
    ...
    if (existingUser)
      return res
        .status(400)
        .json({ message: "Username o email già esistenti" });
    ...
    if (roles.includes("tutor")) {
      ...
      const newTutor = new Tutor({
        ...
        hourlyPrice,
        bio,
        lessonMode: [...new Set(req.body.lessonMode)],
      });
      await newTutor.save(); // Salva il nuovo tutor nel database
    }

    res.status(201).json({
      message: roles.includes("tutor")
        ? "Tutor registrato con successo"
  ...
  try {
    ...
    if (!refreshToken) {
      return res.status(401).json({
        message: "Refresh token mancante. Effettua nuovamente il login.",
      });
    }
    ...
    try {
      ...
      if (!decoded) {
        return res.status(401).json({
          message:
            "Refresh token non valido o scaduto. Effettua nuovamente il login.",
        });
      }
    } catch (error) {
      return res.status(401).json({
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: "Utente non trovato. Effettua nuovamente il login." });
    }
