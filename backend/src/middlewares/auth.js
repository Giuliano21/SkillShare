...
token è presente nell'header di autorizzazione, decodifica le informazioni dell'utente e
...
async function getUserFromAccessToken(token) {
  ...
  if (!userDoc || userDoc.status === "deleted") {
    const error = new Error(
      "L'utente associato a questo token non esiste più. Effettua nuovamente il login.",
    );
